...
import { GreenButton, GreyButton, TextInput } from "..";
...
const ChangePassword = ({
  ...
  const change = () => {
    if (!passRegExp.test(newPass)) {
      ...
      dispatch(openErrorAlert("Новый пароль повторно введен неверно"));
      return;
    } else
      callback(oldPass, newPass)
  };

  return (
    <ModalWindow moduleActive={modalActive} setModuleActive={setModalActive}>
      <div className="reg_auth_header">
        <h1 className="reg_auth_header_title">Смена пароля</h1>
        <svg
          style={{
            padding: "5px",
