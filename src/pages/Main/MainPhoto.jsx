...
import { SelectInput, TextInput, GreenButton, GalleryPhotoPreview } from '../../components';
        ...
        <div className={mapViewActive ? 'main_photo_body' : 'main_photo_body map_disabled'}>
          {!fetching &&
            ...
            photos.map((photo, idx) => (
              <GalleryPhotoPreview
                photo={photo}
                width={window.screen.width <= 576 ? '48%' : mapViewActive ? '258px' : '264px'}
                key={idx}
                wrapperWidth={
                  window.screen.width <= 576 ? '48%' : mapViewActive ? '258px' : '264px'
                }
              />
            ))}
        ...
        {photos && !fetching && photos && photos.length !== 0 && (
          <div className='main_photo_body_pagination'>
            <ThemeProvider theme={theme}>
              <Pagination
                count={mapViewActive ? Math.ceil(countItems / 6) : Math.ceil(countItems / 16)}
                page={page}
                color='secondary'
