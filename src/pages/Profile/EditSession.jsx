...
const EditSession = () => {
  ...
  //получить base64 из урла картинки
  ...
  React.useEffect(() => {
    Requests.getSingleSession(sessionId).then((res) => {
      ...
      res.data.photos.map((photo, idx) => {
        toDataUrl(`https://photodel.ru${photo.photo}`, function (myBase64) {
          sendPhotosArray.push(myBase64); // myBase64 is the base64 string
          setSendPhotosArray(sendPhotosArray);
        });
      });
    });
